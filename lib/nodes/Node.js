...
Node.prototype = assign['default'](Object.create(eventemitter2.EventEmitter2.prototype), {
	...
	_findCreator: function _findCreator() {
		return this;
	},
	...
	createWatchTask: function createWatchTask() {
		var node = this,
		    watchTask,
		    buildScheduled,
		    previousDetails;
	...
	exclude: function exclude(patterns) {
	...
	include: (function (_include) {
		var _includeWrapper = function include(_x) {
	...
	inspect: function inspect(target, options) {
	...
	map: function map(fn, userOptions) {
	...
	serve: (function (_serve) {
		var _serveWrapper = function serve(_x) {
	...
	transform: function transform(fn, userOptions) {
		var options;
	...
	watch: (function (_watch) {
		var _watchWrapper = function watch(_x) {
...
function tryToLoad(plugin) {
	...
	try {
		return requireRelative['default']("gobble-" + plugin, process.cwd());
	} catch (err) {

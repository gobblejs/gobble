...
Node.prototype = {
			...
			node._ready = file.mkdirp( outputDir ).then( function () {
				return new Promise( function ( fulfil, reject ) {
					...
					node.input.ready().then( function ( inputDir ) {
						node.transformer.call( transformation, inputDir, outputDir, node.options, function ( err ) {
							if ( err ) {
								logger.error( 'Transformation failed ({id})', { id: node.id });
								console.log( '============'.grey );
	...
	inspect: function ( target, options ) {
		...
		this.watch( function ( err, inputDir ) {
			if ( !err ) {
				file.copydir( inputDir ).to( target );
			}
		}, { silent: true });
...
function tryToLoad ( plugin ) {
	try {
		...
		if ( err.message === "Cannot find module 'gobble-" + plugin + "'" ) {
			logger.error( 'could not load {name} plugin. Have you installed it? ' + 'npm install --save-dev gobble-{name}'.cyan, { name: plugin });
			err.gobbled = true;
		} else {

...
Node.prototype = {
				...
				return new Promise( function ( fulfil, reject ) {
					...
					node.input.ready().then( function ( inputDir ) {
						node.transformer( inputDir, outputDir, node.options, function ( err ) {
							...
							console.log( 'fulfilling', outputDir, require('fs').readdirSync(outputDir) );
							fulfil( outputDir );
						});
					}).catch( debug );
				});
	...
	copyTo: function ( dest ) {
		return this.ready().then( function ( outputDir ) {
			console.log( 'copying files to %s', dest, require('fs').readdirSync( outputDir ) );
			return file.copydir( outputDir, dest );
		});
	}
};

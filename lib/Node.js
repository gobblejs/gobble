var fs = require( 'graceful-fs' ),
	...
	queue = require( './queue' ),
	loggedMapWarning = false; // TODO remove this after a few versions...
...
var Node = function ( input, transformer, options, id ) {
	...
	node.cachedir = path.join( node.outputDir, '.cache' );
...
Node.prototype = {
				...
				return new Promise( function ( fulfil, reject ) {
					...
					node.input.ready().then( function ( inputDir ) {
						queue.add( function ( cb ) {
							var callback, errback, start, subtask;

							subtask = messenger.start( '%s transformation running...', node.id );
							start = Date.now();

							callback = function () {
								subtask.done();
								messenger.info( '%s transformation finished in %sms', node.id, Date.now() - start );
								fulfil( outputDir );
								cb();
							};
							...
							try {
								node.transformer.call( transformation, inputDir, outputDir, assign({}, node.options ), callback, errback );
							} catch ( err ) {
								errback( err );
							}
						});
	...
	transform: function ( fn, userOptions ) {
		...
		// If function takes fewer than 4 arguments, it's a file transformer
		if ( fn.length < 4 ) {
	...
	map: function ( fn, userOptions ) {
		if ( !loggedMapWarning ) {
			console.log( 'node.map() is deprecated. You should use node.transform() instead for both file and directory transforms' );
			loggedMapWarning = true;
		}
	...
	_cleanup: function () {
		...
		fs.readdirSync( node.outputDir ).map( function ( dir ) {
			...
			return !/\.cache/.test( dirpath ) && dirpath !== node._lastOutputDir;

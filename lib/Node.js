...
Node.prototype = {
	...
	include: function ( patterns ) {
		if ( typeof patterns === 'string' ) { patterns = [ makeRegex( patterns ) ]; }
		return new Node( this, builtins.include, { patterns: patterns.map( makeRegex ) });
	},

	exclude: function ( patterns ) {
		if ( typeof patterns === 'string' ) { patterns = [ makeRegex( patterns ) ]; }
		return new Node( this, builtins.include, { patterns: patterns.map( makeRegex ), exclude: true });
	},
...
function makeRegex ( string ) {
	return minimatch.makeRe( string );
}

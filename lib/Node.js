...
Node.prototype = {
	...
	transform: function ( transformer, options ) {
		if ( typeof transformer === 'string' ) {
			transformer = requireRelative( 'gobble-' + transformer, cwd() );
		}

		if ( transformer.length !== 4 ) {
			console.log( 'GOBBLE WARNING: Wrong function length - did you mean to use tree.map() instead of tree.transform()? See https://github.com/gobblejs/gobble/wiki/Troubleshooting for more information\n' );
		}
	...
	map: function ( fn, options ) {
		...
		if ( typeof fn === 'string' ) {
			fn = requireRelative( 'gobble-' + fn, cwd() );
		}

		if ( fn.length !== 2 ) {
			console.log( 'GOBBLE WARNING: Wrong function length - did you mean to use tree.transform() instead of tree.map()? See https://github.com/gobblejs/gobble/wiki/Troubleshooting for more information\n' );
		}

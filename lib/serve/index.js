module.exports = function ( node, options ) {
	...
	templates.dir = (function () {
		return helpers.readFile( path.join( __dirname, 'templates/dir.html' ) ).then( function ( t ) {
			return template( t );
		});
	}());
	...
	node.watch( function ( srcDir ) {
		console.log( 'folder changed:', srcDir );

		server = http.createServer();
		server.listen( port );

		server.on( 'request', function ( request, response ) {
			var filepath = path.join( srcDir, request.url );

			helpers.stat( filepath ).then( function ( stats ) {
				if ( stats.isDirectory() ) {
					// TODO serve index.html or dir view
		...
		function serveFile ( filepath, request, response ) {
			return helpers.readFile( filepath ).then( function ( data ) {
				response.statusCode = 200;

	...
	sander.lsr( srcDir ).then( function ( files ) {
		var promises = files.map( function ( filename ) {
			...
			if ( options.accept && !~options.accept.indexOf( ext ) ) {
				return sander.copyFile( srcDir, filename ).to( destDir, filename );
			}

			return sander.stat( srcDir, filename ).then( function ( stats ) {
				if ( stats.isDirectory() ) {
					return;
				}

				return sander.readFile( srcDir, filename ).then( function ( data ) {
					var crc, previous;
					...
					if ( previous && compareBuffers( crc, previous.crc ) ) {
						return sander.copyFile( previous.path ).to( destDir, filename );
					}
					...
					return new Promise( function ( fulfil, reject ) {
						...
						queue.add( function ( cb ) {
							try {
								result = options.fn( data.toString(), assign( {}, options.userOptions ) );
							} catch ( e ) {
								if ( typeof e === 'string' ) {
									err = new Error( e );
								} else {
